...
export const LocalSearchField: React.FC<LocalSearchFieldProps> = ({searchFieldProps}) => {
  ...
  const search = useCallback(() => {
    ...
    localSearchClientWrapper.client?.searchCode(program, searchTerm, fileRef, searchFieldProps.max, codeHost, owner, repo, currentSearchRef, (current, total) => setProgress({
      current,
      total
    }))
  ...
  return <>
    ...
    {progress !== undefined && <div><Box sx={{paddingTop: 1, width: '100%'}}>
      {searchFieldProps?.state === 'searching' && currentSearchRef.current !== 0 &&
        ...
        <LinearProgress title={'progress'} color={"primary"} value={progress.current / progress.total * 100}
                        variant={"determinate"}/> {progress.current}/{progress.total}
    </Box></div>}
