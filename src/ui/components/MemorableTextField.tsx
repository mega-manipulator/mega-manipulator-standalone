...
export const MemorableTextField: React.FC<CombinedProps> = ({
  memProps,
  textProps,
}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const retrieved: string[] | null = await store.get(
        memProps.megaFieldIdentifier
      );
  ...
  const internalEnterAction = useCallback(() => {
    ...
    store
      ...
      .then(() =>
        debug(
          `Updated ${memProps.megaFieldIdentifier} storage with [${tmp.join(
            ','
          )}]`
        )
      )
      .catch(
        (e) =>
          `Failed updating file store for MemorableTextField '${
            memProps.megaFieldIdentifier
          }': ${asString(e)}`
      );
