          ...
          <p>
            {selected.map((a, idx) => (
              <>
                ...
                <Tooltip title={'Reset entire repo (TODO 🤦)'}>
                  <IconButton
                    onClick={() => window.alert(`It's on my todo-list!`)}
                  >
                    <RestartAltIcon />
                  </IconButton>
                </Tooltip>
                {stagedFiles[idx] ? (
                  <>
                    <Typography color={'#6A6'}>
                      Staged files ({stagedFiles[idx].length}):{' '}
                    </Typography>
                    {stagedFiles[idx].map((s) => (
                      <>
                        ...
                        <Tooltip title={'Reset file (TODO 🤦)'}>
                          <IconButton
                            onClick={() =>
                              window.alert(`It's on my todo-list!`)
                            }
                          >
                            <RestartAltIcon />
                          </IconButton>
                ...
                {unStagedFiles[idx] ? (
                  <>
                    <Typography color={'#A66'}>
                      UnStaged files ({unStagedFiles[idx].length}):{' '}
                    </Typography>
    ...
    (closeCallback) => (
      <ButtonRow>
        ...
        <Button
          ...
          onClick={() => {
            gitStage(
              new GitStageInput(settings, selected, () => {
                return;
              })
            )
              .then(loadStagingInfo)
              .catch((e) =>
                error(`Something failed staging files: ${asString(e)}`)
              );
          }}
        >
          <AddIcon />
        ...
        <Button
          ...
          onClick={() => {
            ...
            gitUnStage(
              ...
              .then(loadStagingInfo)
              .catch((e) =>
                error(`Something failed un-staging files: ${asString(e)}`)
              );
          }}
        >
          <RemoveIcon />
