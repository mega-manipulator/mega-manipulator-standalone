...
) => LocalSearchClientWrapper = (settings: MegaSettingsType) => {
  const [wrapper, setWrapper] = useState<LocalSearchClientWrapper>({
    client: undefined,
...
export class LocalSearchClient {
  ...
  async searchCode(
    ...
    let pathParts: string[] = [
      this.megaSettings.keepLocalReposPath,
      codeHost,
      owner,
      repo,
    ];
    ...
    debug(
      `Search for '${searchString}' with the local client in ${keeps.length} dirs, using '${program}'`
    );
    ...
    chunk: for (let i = 0; i < keeps.length; i += chunkSize) {
      ...
      const commands: (SearchHit | null)[] = await Promise.all(
        thisChunk.map((keep) => {
          return withTimeout(
            1000,
            searchCommand(program, searchString, fileString, keep)
          ).then(async (c) => {
            if (c.code === 0) {
