...
import { deleteResultFromStorage, getResultFromStorage, listResultInStorage, pruneOldestResultsFromStorage } from '../../service/work/workLog';
import { WorkResult, WorkResultOutput, WorkResultStatus } from '../../service/types';
import { DataGridPro, GridColDef } from '@mui/x-data-grid-pro';
import { Alert, AlertColor, Box, Button, Checkbox, FormControl, FormHelperText, IconButton, Modal, Tooltip, Typography } from '@mui/material';
...
export const ResultPage: React.FC = () => {
  ...
  const columns: GridColDef[] = [
    { field: 'id', hideable: true, hide: true, width: 150 },
    {
      ...
      width: 150,
      renderCell: (e) => (
        <Typography style={{ cursor: 'pointer' }} onClick={() => nav(`${locations.result.link}/${e.value}`)}>
          {e.value}
        </Typography>
      ),
    },
  ...
  return (
    <>
      ...
      <Box sx={{ height: 400, width: '100%' }}>
        <DataGridPro rows={resultList} columns={columns} pageSize={15} rowsPerPageOptions={[5, 15, 100]} checkboxSelection disableSelectionOnClick />
      </Box>
      <div>
        <Tooltip title={'No warning, or confirm dialog, just BAM!'}>
          <Button variant={'outlined'} color={'warning'} onClick={() => pruneOldestResultsFromStorage(100).then(() => setReload(reload + 1))}>
...
const ResultTable: React.FC<ResultTableProps> = ({ work }: ResultTableProps) => {
  ...
  return (
    <>
      ...
      <div>
        <Alert color={statusToColor(work.status)}>Full Result: {work.status}</Alert>
      </div>
      <FormControl>
        <FormHelperText>
          <Typography>Show raw JSON</Typography>
        </FormHelperText>
        <Checkbox checked={showJson} onClick={() => setShowJson(!showJson)} />
      </FormControl>
      {showJson && (
        <>
          <hr />
...
const ResultItem: React.FC<ResultItemProps> = ({ input, output }) => {
  ...
  return (
    <div onClick={() => setFatJson(!fatJson)}>
      <div>Input:</div>
      <Typography>{JSON.stringify(input, null, fatJson ? 2 : undefined)}</Typography>

      <div>Output:</div>
      <Typography>{JSON.stringify(output.meta, null, fatJson ? 2 : undefined)}</Typography>
