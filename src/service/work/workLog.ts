...
export async function runCommand(
  ...
  const result: ChildProcess = await new Command(program, args, {
    cwd: dir,
  }).execute();
  if (meta) {
    ...
    await trace(
      `=> Ran '${logEntry.what}' in ${dir} with result ${JSON.stringify(
        result
      )}`
    );
  }
...
export async function saveResultToStorage(
  result: WorkResult<unknown, unknown, unknown>
) {
  await store.set(`${result.time}`, result);
}

export async function getResultFromStorage(
  time: string
): Promise<WorkResult<unknown, unknown, unknown> | null> {
...
export async function pruneOldestResultsFromStorage(leave: number) {
  ...
  if (oldes.length > 0) {
    info(
      `Will now delete ${oldes.length} oldest results from storage, keeping ${leave}`
    );
    ...
    info(
      `Will not delete anything. Asked to keep ${leave} and only had ${allKeys.length}`
    );
  }
}
