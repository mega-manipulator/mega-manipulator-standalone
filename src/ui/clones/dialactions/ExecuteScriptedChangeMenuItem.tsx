...
export function useExecuteScriptedChangeMenuItem(): GenericSpeedDialActionProps {
  ...
  const action = useCallback(
    async (progress: ProgressReporter) => {
      switch (runMode) {
        case 'parallel':
          return await runScriptInParallel(
            { settings, filePaths: selected },
            progress
          );
        case 'sequential':
          return await runScriptSequentially(
            { settings, filePaths: selected },
            progress
          );
      }
    ...
    selected.length === 0 ? (
      ...
      <>
        <Typography variant={'h4'}>
          Run Scripted Change on {selected.length} projects?
        </Typography>
        <Typography>
          The script will execute in the root of every project folder, and can
          be run in sequence or in parallel.
        </Typography>
        <FormControl>
          ...
          <Switch
            ...
            onClick={() =>
              setRunMode(runMode === 'parallel' ? 'sequential' : 'parallel')
            }
          />
        </FormControl>

        <Tooltip title={'Open change-script'}>
          <IconButton
            onClick={() =>
              path
                .join(settings.clonePath, scriptFile)
                .then((file) => open(file))
            }
          >
            <FileOpenIcon />
