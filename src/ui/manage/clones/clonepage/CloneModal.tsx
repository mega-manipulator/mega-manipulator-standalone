...
export const CloneModal: React.FC<CloneModalPropsWrapper> = ({
  ...
  useEffect(() => {
    setBranch(
      'batch_' + new Date().toISOString().replaceAll(/[^a-zA-Z0-9-]/g, '_')
    );
  }, []);
  ...
  return (
    <Modal
      open={cloneModalPropsWrapper.isOpen}
      onClose={() => {
      ...
      <Box sx={modalStyle}>
        ...
        {selected.length !== 0 && state === 'ready' && (
          <>
            <div>
              <Typography>
                Clone {selected.length} things into {settings?.clonePath}?
              </Typography>
            </div>
            {selected.some((s) => s && s.branch === undefined) && (
              <div>
                <FormControl>
            ...
            <div>
              <Tooltip
                title={
                  "Run 'git fetch' on the clones found locally, but it's faster not to run the fetch"
                }
                arrow
              >
                <FormControl>
                  <FormHelperText>
                    <Typography>Fetch if exists locally?</Typography>
                  </FormHelperText>
                  <Switch
                    checked={fetchIfLocal}
                    onClick={() => setFetchIfLocal(!fetchIfLocal)}
                  />
                </FormControl>
              </Tooltip>
            </div>
            <div>
              <Tooltip title={'Only clone to keep dir.'} arrow>
                <FormControl>
                  <FormHelperText>
                    <Typography>Skip workdir copy?</Typography>
                  </FormHelperText>
                  <Switch
                    checked={onlyKeep}
                    onClick={() => {
            ...
            <div>
              <FormControl fullWidth>
                ...
                <Switch
                  checked={doSparseCheckout}
                  onClick={() => setDoSpaseCheckout(!doSparseCheckout)}
                />
              </FormControl>
              {doSparseCheckout && (
                <MemorableTextField
        ...
        {(state === 'running' || state === 'done') && (
          <>
            ...
            {progress?.breakdown &&
              Object.keys(progress?.breakdown).map((k) => (
                <>
                  {k}: {progress.breakdown[k]},&nbsp;
                </>
              ))}
          </>
        )}
