 ...
 * @param action
 */
export async function simpleActionWithResult(
  ...
  for (let i = 0; i < workResult.result.length; i++) {
    ...
    await action(
      ...
      p,
      meta,
      (sts: WorkResultStatus) => (workResult.result[i].output.status = sts)
    );
  }
...
async function validClonePath(settings: MegaSettingsType): Promise<string> {
  ...
  if (!clonePath.startsWith(home))
    throw new Error('Clone/work-path is not in your home dir');
  return clonePath;
}

export function requireZeroStatus(
