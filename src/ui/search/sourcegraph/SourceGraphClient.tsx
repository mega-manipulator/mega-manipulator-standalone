    ...
    try {
      if (repoString) {
        ...
        if (parts.length === 3) {
          ...
          const codeHostKey: string | undefined =
            this.searchSettings.codeHosts[sgCodeHost];
...
export function useSourceGraphClient(
  ...
  const [wrapper, setWrapper] = useState<SourceGraphClientWrapper>({
  ...
  useEffect(() => {
    (async () => {
      if (!searchHostKey) {
        setWrapper({
          error: 'searchHostKey not set/initialized',
          client: undefined,
        });
        return;
      } else if (!props) {
        ...
        setWrapper({
          error: 'settings not set/initialized',
          client: undefined,
        });
        return;
      }
      const sourceGraphSettings =
        settings?.searchHosts[searchHostKey]?.sourceGraph;
      if (!sourceGraphSettings) {
        setWrapper({
          error: 'sourceGraphSettings is not defined',
          client: undefined,
        });
        return;
      }
