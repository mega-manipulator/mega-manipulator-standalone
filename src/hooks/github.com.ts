...
export class GithubClient {
  ...
  async prDraftOrReadyForReview(input: { prs: GitHubPull[]; draft: boolean }, progressCallback: (idx: number) => void): Promise<WorkResult<unknown, GitHubPull, WorkMeta>> {
    ...
    try {
      const resp = await this.evalRequest(name, meta, () =>
  ...
  async reviewPullRequests(
    input: {
      prs: GitHubPull[];
      body: { body: string; event: 'REQUEST_CHANGES' | 'APPROVE' };
    },
  ...
  private async patchPullRequests(
    input: {
      prs: GitHubPull[];
      comment?: string;
      dropBranch?: boolean;
  ...
  private async createPullRequest(hit: SearchHit, title: string, body: string, head: string, base: string, meta: WorkMeta) {
    await this.evalRequest('POST Create PullRequest', meta, async () => {
      ...
      return await this.api.post(
        `/repos/${hit.owner}/${hit.repo}/pulls`,
        {
          title,
          body,
          head,
        ...
        case 'ok': {
          ...
          mapp: for (const item of mapped) {
            for (const present of aggregator) {
              if (equality(present, item)) continue mapp;
            }
            aggregator.add(item);
            if (aggregator.size === max) break pagination;
          }
