...
import {
  GitHubCodeHostSettings,
  GitHubSearchHostSettings,
  MegaSettingsType,
} from '../../../hooks/settings';
...
async function bakeGithubClient(
  ...
  if (!baseUrl) {
    return {
      clientInitError: 'BaseUrl not set, go back to settings to set it',
    };
  }
  const username = settings.username;
  if (!username) {
    return {
      clientInitError: 'Username not set, go back to settings to set it',
    };
  }
  const token = await getPassword(username, baseUrl);
  if (!token) {
    return {
      clientInitError: 'Token not saved, go back to settings to set it',
    };
  }
  return {
...
export const useGitHubSearchClient: () => GithubClientWrapper = () => {
  const {
    settings,
    search: { searchHostKey },
  } = useContext(MegaContext);
  const [wrapper, setWrapper] = useState<GithubClientWrapper>({
    clientInitError: 'Not initialized',
  });
  useEffect(() => {
    (async () => {
      if (!searchHostKey) {
      ...
      const hostSetting: GitHubSearchHostSettings | undefined =
        settings.searchHosts[searchHostKey]?.github;
      if (!hostSetting) {
        setWrapper({ clientInitError: 'Undefined search host settings 🤔' });
        return;
      }
      setWrapper(
        await bakeGithubClient(
          searchHostKey,
          hostSetting.codeHostKey,
          settings,
          hostSetting
        )
      );
...
export const useGitHubCodeClient: () => GithubClientWrapper = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const hostSetting: GitHubCodeHostSettings | undefined =
        settings.codeHosts[codeHostKey]?.github;
      if (!hostSetting) {
        setWrapper({ clientInitError: 'Undefined search host settings 🤔' });
        return;
      }
      setWrapper(
        await bakeGithubClient('??', codeHostKey, settings, hostSetting)
      );
    })();
