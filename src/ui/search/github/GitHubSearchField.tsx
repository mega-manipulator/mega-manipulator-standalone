...
export const GitHubSearchField: React.FC<GitHubSearchFieldProps> = ({
  searchFieldProps,
}) => {
  const {
  ...
  const { ghClient, clientInitError } = useGitHubSearchClient();
  useEffect(() => {
  ...
  if (clientInitError) {
    return (
      <Alert variant={'filled'} color={'error'}>
        Failed setting up search client: {clientInitError}
      </Alert>
    );
  }
  return (
    <>
      ...
      <Button
        ...
        color={'primary'}
        disabled={
          searchFieldProps?.state !== 'ready' || searchTerm.length === 0
        }
        onClick={search}
      >
        {searchFieldProps?.state === 'searching' && (
      ...
      {docLink && (
        <Tooltip
          title={`Click to open ${searchType} search documentation in browser`}
        >
          <IconButton
            onClick={() => {
              debug('Opening docs');
