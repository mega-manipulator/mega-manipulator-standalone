...
import {
  MegaSettingsType,
  SourceGraphSearchHostSettings,
} from '../../hooks/settings';
import { MegaContext } from '../../hooks/MegaContext';

function codeHostStyle(
  key: string,
...
export const SourceGraphSearchHostSettingsPage: React.FC = () => {
  ...
  const settings: SourceGraphSearchHostSettings | null =
    useMemo<SourceGraphSearchHostSettings | null>(() => {
  ...
  const [searchHostEdit, updateSearchHost, setSearchHost] =
    useMutableState<SourceGraphSearchHostSettings>(defaultSourceGraphSettings);
  useEffect(() => {
    if (settings) {
  ...
  useEffect(() => {
    ...
    if (
      !searchHostEdit?.baseUrl ||
      !searchHostEdit.baseUrl.match(/^https?:\/\/.*[^/]$/g)
    ) {
    ...
    if (
      !searchHostEdit?.codeHosts ||
      Object.keys(searchHostEdit.codeHosts).length === 0
    ) {
      ...
      Object.values(searchHostEdit.codeHosts).forEach((codeHost) => {
        if (!megaSettings?.codeHosts[codeHost]) {
          warnAggregate.push(
            `Incorrect code host mapping, code host '${codeHost}' does not exist`
          );
        }
      });
    }
  ...
  return (
    <>
      ...
      <div>
        {errors.map((err, idx) => (
          <Alert key={idx} variant={'filled'} severity={'error'}>
            {err}
          </Alert>
        ))}
        {warnings.map((w, idx) => (
      ...
      <FormControl>
        <FormHelperText>
          User name (Only used for multi-user password storage reasons)
        </FormHelperText>
        <TextField
      ...
      <div>
        <Table>
          ...
          <TableBody>
            {searchHostEdit?.codeHosts &&
              Object.keys(searchHostEdit.codeHosts).map((key, index) => (
                <TableRow key={index}>
                  <TableCell>{key}</TableCell>
                  <TableCell
                    style={codeHostStyle(
                      searchHostEdit.codeHosts[key],
                      megaSettings
                    )}
                  >
                    {searchHostEdit.codeHosts[key]}
        ...
        <Button
          ...
          onClick={() => {
            updateSearchHost((draft) => {
              if (
                newMappingKey &&
                newMappingKey.length > 0 &&
                newMappingValue &&
                newMappingValue.length > 0
              ) {
