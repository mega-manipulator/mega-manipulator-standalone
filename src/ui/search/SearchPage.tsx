...
import {
  ...
  useCloneModalProps,
} from '../manage/clones/clonepage/CloneModal';
...
export const SearchPage: React.FC = () => {
  ...
  const {
    settings,
    search: { selected, searchHostKey, setSearchHostKey },
  ...
  const searchHostSelect = (
    <FormControl>
      ...
      >
        {settings &&
          ['LOCAL', ...Object.keys(settings.searchHosts)].map((k, i) => (
  ...
  return (
    <>
      ...
      <Backdrop
        open={searchFieldProps?.state === 'loading'}
        sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }}
      >
        {settings &&
        settings.searchHosts &&
        Object.keys(settings.searchHosts).length > 0 ? (
          <Box sx={modalStyle}>
            <CircularProgress />
            <div>{searchHostSelect}</div>
            <Button
              variant={'contained'}
              color={'warning'}
              onClick={() => nav(locations.settings.link)}
            >
              Take me back to the Settings page
            </Button>
          </Box>
        ) : (
          <ErrorBoundary />
      ...
      {searchType === 'LOCAL' && (
        <LocalSearchField searchFieldProps={searchFieldProps} />
      )}
      {searchType === 'GITHUB' && (
        <GitHubSearchField searchFieldProps={searchFieldProps} />
      )}
      {searchType === 'SOURCEGRAPH' && (
        <SourceGraphSearchField searchFieldProps={searchFieldProps} />
      )}

      {searchFieldProps?.state === 'searching' ? (
        <Alert severity={'info'}>Search in progress</Alert>
      ) : (
        <Button
          variant={'contained'}
          color={'info'}
